...
Action parse_value_expression(TokenList& token_list);
...
//  Return true if the first element of the given list is the given token.
    ...
    bool lookahead_far(TokenList& token_list, TokenKey target_token, int distance) {
        if (token_list.empty()) {
            return false;
        }
...
//  Return true if the first element of the given list is any of the given tokens.
    ...
    bool lookahead_far_any(TokenList& token_list, TokenKeyArray& target_tokens, int distance) {
        if (token_list.empty()) {
            return false;
        }
        
        for (int target_index = 0; target_index < target_tokens.size(); target_index++) {
            if (lookahead_far(token_list, target_tokens[target_index], distance)) {
                return true;
            }
        }
        return false;
...
//  Check for and parse an expression encased in parenthesis '()'.
    if (lookahead(token_list, LeftPar)) {
        Action expression;
...
//      Bypass and store the encased expression.
        ...
        return parse_value_expression(token_list);
    }
}
